...
void Bullet::update(float dt)
{
    ...
    if (bulletKind != 2 || !collidedJudge) direction = monsterTarget->getParent()->convertToWorldSpace(monsterTarget->getPosition()) - absolutePos;
    //转动子弹指向怪物
    ...
    //移动速度（一个矢量）
    Vec2 velocity;
    if (bulletKind != 2)velocity = /*direction.getNormalized()*/Vec2(0, 1) * speed * dt;
    else velocity = direction.getNormalized() * speed * dt;
    CCLOG("position:(%.2f,%.2f)", monsterTarget->getPosition().x, monsterTarget->getPosition().y);

    if (distance <= 80 && !collidedJudge)//子弹接触到怪时
    {
        collidedJudge = true;//标记为已碰撞过

        monsterTarget->setHealth(monsterTarget->getHealth() - damage);//对怪物造成伤害
        if (monsterTarget->getSpeed() > 30.0f) {
            monsterTarget->setSpeed(monsterTarget->getSpeed() - slowDown);//对怪物减速
        }
        if(bulletKind==3) monsterTarget->isPoisoned = 1;

        auto healthBar = monsterTarget->HealthBar();//获取怪物血条
